...
namespace AutoSkelbiuScraper
{
    ...
    class Program
    {
        ...
        private static List<Auto> auto = new List<Auto>();
        private static List<LINKS> l = new List<LINKS>();
        private static int userAgentRotation = 0;
        private static int requestCount = 300;
        ...
        private static void GetUrlList()
        {
            using (MySqlConnection connection = new MySqlConnection("server=localhost;user=scraperis;password=useris3;database=AutoSkelbiu;"))
            {
                connection.Open();
                l = new List<LINKS>();
                l = connection.Query<LINKS>("SELECT * FROM LINKS WHERE IS_SOLD=0 AND SOURCE='autogidas' ORDER BY CREATED_AT DESC").ToList();
            }
        }
        ...
        static Auto GetCarMakeAndPrice(Auto auto, string html)
        {
            ...
            try
            {
                ...
                HtmlNodeCollection nodeCollection = doc.DocumentNode.SelectNodes("//div[@class='param']");
        ...
        static private void GetImageGallery(int linkId, string html)
        {
            try
            {
                string[] galleryItems = html.Split(new string[] { "gallery.addImage('" }, StringSplitOptions.None);

                List<Image> imagesFromWeb = new List<Image>();

                for (int i = 1; i < galleryItems.Length; i++)
                {
                    Image img = new Image();
                    img.URL = galleryItems[i].Split("'")[0];
                    imagesFromWeb.Add(img);
                }
        ...
        static private string GetParameters(string html, int linkId)
        {
            ...
            string[] arr = new string[] { "Metai", "Markė", "Modelis", "Rida, km", "Variklis",
            "Kuro tipas", "Kėbulo tipas", "Durų skaičius", "Pavarų dėžė",
            "Klimato valdymas","Spalva",
            "Vairo padėtis", "Ratlankiai",
            "Sėdimų vietų skaičius", "CO₂ emisija, g/km", "TA iki",
            "Ilgis", "Aukštis", "Varomieji ratai", "Defektai",
             "SDK" };
            ...
            for (int i = 0; i < arr.Length; i++)
            {
                try
                {
                    ...
                    string delimeter = "</div><div class=\"right\">";
                    ...
                    {
                        case "Markė":
                            auto.AUTO_MAKE = parameters;
                            break;
                        case "Modelis":
                            auto.AUTO_MODEL = parameters;
                            break;
                        case "Metai":
